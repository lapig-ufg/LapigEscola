...
function createStyle(styleConfig = {}) {
  const fill = styleConfig.fill
    ? new Fill({ color: styleConfig.fill.color || "rgba(238, 238, 238, 0.2)" })
    : undefined;
  ...
  const image = styleConfig.marker
    ? new Circle({
...
export default function useOpenLayers(
      ...
      switch (layerConfig.layer_type) {
        ...
        case "WMS":
          olLayer = new TileLayer({
            source: new TileWMS({
              url: layerConfig.url,
              params: (any = config.metadata?.params || {
                LAYERS: "default",
                TILED: true,
              }),
        ...
        default:
          console.warn(
            `Tipo de camada desconhecido: ${layerConfig.layer_type}`
          );
          return;
      }
  ...
  const fitToFeatureUrl = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Falha ao buscar GeoJSON de foco: ${response.statusText}`
        );
      }
      ...
      if (olFeatures.length > 0) {
        ...
        console.warn(
          `A URL de foco (${focusFeatureUrl.value}) não retornou nenhuma feature.`
        );
      }
    } catch (error) {
      console.error(
        `Erro ao processar a URL do GeoJSON de foco (${focusFeatureUrl.value}):`,
        error
      );
    } finally {
